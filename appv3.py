...
if pdf is not None:
    ...
    query_option = st.selectbox("Choose an option", ["Which job roles align most effectively with my skill set?",], index=None)
    ...
    query_option2 = st.selectbox("Choose an option", ["Suggest addition skills needed to mactch each job role",], index=None)

    # query = st.text_input("Ask questions")

    if query_option2 is not None:
        # search the knowledge base for document
        docs = knowledge_base.similarity_search(query_option2)
        ...
        #run the chain with input document and user option
        response = chain.run(input_documents = docs, question=query_option2)
