...
if pdf is not None:
    ...
    data = ResumeParser("C:/Users/DELL/OneDrive/Desktop/Job Application in Hull/Data_Engineer/Big_Data_Engineer.pdf").get_extracted_data()

    # Display the extracted information
    st.text(f"Name: {data.get('name')}")
    st.text(f"Email: {data.get('email')}")
    st.text(f"Phone: {data.get('mobile_number')}")
    st.text(f"Skills: {', '.join(data.get('skills', []))}")
    st.text(f"Experience: {data.get('experience')}")
    st.text(f"Education: {data.get('degree')}")
